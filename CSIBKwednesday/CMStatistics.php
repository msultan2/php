...
;
        ...
        $sqlTable2 = "  Select CRQ,`Activity Owner`,Postponed,round(Total_SPEECH_TRAFFIC/1000,2) Total_SPEECH_TRAFFIC_KErlg,
...
from
(SELECT CRQ,`Activity Owner`,Postponed,Hour_From,Hour_To,sum(`Activity Log`.Total_SPEECH_TRAFFIC) Total_SPEECH_TRAFFIC,sum(`Activity Log`.DATA_MB) DATA_MB,count(SiteID) Site_Count
FROM `Activity Log`
WHERE  date_format(`Activity Date`,'%Y-%m-%d')='".$ChangeStatisticsDate."'
group by CRQ,Hour_From,Hour_To) Date_Data,
...
if ($resultTable2->num_rows > 0) {
	...
	while($row = $resultTable2->fetch_assoc()) {
		...
		*/
		$question = array(
		'CRQ' => $row["CRQ"],
		'Total_SPEECH_TRAFFIC_KErlg' => $row["Total_SPEECH_TRAFFIC_KErlg"]
