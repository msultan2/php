...
$sql=mysql_query(" select substr(`Desc`,1,4) as `Desc` ,Count from DUAL_2G where Hour=$hour and Date='$date' and (`Desc` like '2 VF%' or `Desc` like 'Number of Dual Handsets in the Network') order by `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['Count'];
