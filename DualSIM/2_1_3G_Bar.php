...
$sql=mysql_query("select trim(substr(`Desc`,15,21)) as `Desc` ,Count from TWOSIM_3G where Hour=$hour and Date='$date' and (`Desc` not like '%not Populated' and `Desc` not like '%NULL%') order by `Desc` ");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['Count'];
