...
$sql=mysql_query(" SELECT substr(`Desc`,1,4) as `Desc`,  sum(Count) counts FROM (SELECT `Desc`, Count FROM DUAL_3G WHERE Date = '$date' and Hour=$hour and ( `Desc` like '2 VF%' or `Desc` like 'Number of Dual Handsets in the Network')UNION ALL SELECT `Desc`, Count FROM DUAL_2G WHERE Date = '$date' and Hour=$hour and  (`Desc` like '2 VF%'or `Desc` like 'Number of Dual Handsets in the Network') ) X GROUP BY `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['counts'];
