...
$sql=mysql_query("select trim(substr(`Desc`,25,21)) as `Desc` ,sum(Count) counts FROM (SELECT `Desc`, Count FROM TWOSIM_2G WHERE Date = '$date' and Hour=$hour and (`Desc` not like '%not Populated' and `Desc` not like '%Mobile' and `Desc` not like '%NULL%') union All select `Desc`, Count FROM TWOSIM_3G WHERE Date = '$date' and Hour=$hour and (`Desc` not like '%not Populated' and `Desc` not like '%Mobile' and `Desc` not like '%NULL%')) X GROUP BY `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['counts'];
...
$title="Ratio of [2/1] Duals Regionalized on $date at hour= $hour";
