...
$sql=mysql_query("select substr(`Desc`,11,3) as `Desc` , sum(Count) counts from(select `Desc`, Count from TWOSIM_2G where Date='$datei' and `Desc` like '%2 SIMs in One Mobile') X group by `Desc`");
while($rows=mysql_fetch_array($sql)):
