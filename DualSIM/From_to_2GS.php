...
$datei=$new[$i];
//echo "$new[$i]";
$sql=mysql_query("select substr(`Desc`,1,4) as `Desc` , sum(Count) counts from(select `Desc`, Count  from DUAL_2G  where Date='$datei' and (`Desc` like '2 VF%' or `Desc` like '%Number of Dual Handsets in the Network' )) X group by `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['counts'];
array_push($v2,"$sup");
