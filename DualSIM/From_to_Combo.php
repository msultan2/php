...
$dt=array();
$s2="";
$sql=mysql_query("select distinct(Date) as dates from TWOSIM_2G where Date between'$datef' and '$datet'");
...
$sql=mysql_query("select substr(`Desc`,11,3) as `Desc` , sum(Count) counts from(select `Desc`, Count from TWOSIM_2G where Date='$datei' and `Desc` like '%2 SIMs in One Mobile' union all select `Desc`, Count from TWOSIM_3G where Date='$datei' and `Desc` like '%2 SIMs in One Mobile') X group by `Desc`");
while($rows=mysql_fetch_array($sql)):
...
if ($datet == $datef){
$title="Subscribers swapping 2 Vodafone SIMs per one Mobile on $datef";}
else {
$title="Subscribers swapping 2 Vodafone SIMs per one Mobile between $datef and $datei";
} 
