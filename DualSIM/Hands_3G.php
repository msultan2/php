...
global $datef;
global $datet;
$sql=mysql_query("select DISTINCT Date as Date from DUAL_3G order by Date "); //where Hour=$hour and Date='$date'
...
<?php 
$sql=mysql_query("select DISTINCT Hour as Hour from DUAL_3G order by Hour");
while($rows=mysql_fetch_array($sql)):
$hour1=$rows['Hour']; 
...
$chart=$r1;
Print '<img src="Handsets_Bar_3G.php?date=' . $date . '&hour=' . $hour . '&chart='. $chart .'" id="imggg">';}
else
{
$date=$date1;
$sql=mysql_query("select DISTINCT Hour as Hour from DUAL_3G where Date='$date1' order by Hour");
while($rows=mysql_fetch_array($sql)):
$hour1=$rows['Hour']; 
endwhile;
...
if ($date!="" and $hour!="" and $date!='Date' and $hour!='Hour'){
$chart=$r2;
Print '<img src="Handsets_Dual_PIE_3G.php?date=' . $date . '&hour=' . $hour . '&chart='. $chart .'" id="imggg">';}
else
{
$date=$date1;
...
$chart=$r1;
Print '<img src="Handset_TotalDay_Bar_3G.php?date=' . $date . '&chart='. $chart .'" id="imggg">';}
...
$chart=$r1;
Print '<img src="Handset_From_to_Combo_3G.php?datef=' . $datef . '&datet=' . $datet . '&chart='. $chart .'" id="imggg">';  }
