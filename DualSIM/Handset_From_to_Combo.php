...
$datei=$new[$i];
//echo "$new[$i]";
$sql=mysql_query("select substr(`Desc`,1,4) as `Desc` ,sum(Count) counts FROM (SELECT `Desc`, Count FROM DUAL_2G WHERE Date = '$datei' and (`Desc` like '2 VF%' or `Desc` like  '%Number of Dual Handsets in the Network' )   union All select `Desc`, Count FROM DUAL_3G WHERE Date = '$datei' and (`Desc` like '2 VF%' or `Desc` like  '%Number of Dual Handsets in the Network' ) ) X GROUP BY `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['counts'];
array_push($v2,"$sup");
...
$b1plot->value->Show();
$b2plot->value->Show();
$b2plot->SetLegend('Dual Transactions');
$b1plot->SetLegend('Normal Transactions');
