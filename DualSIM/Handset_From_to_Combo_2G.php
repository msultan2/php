...
$dt=array();
$s2="";
$sql=mysql_query("select distinct(Date) as dates from DUAL_2G where Date between'$datef' and '$datet'");
...
$datei=$new[$i];
//echo "$new[$i]";
$sql=mysql_query("select substr(`Desc`,1,4) as `Desc` , sum(Count) counts from(select `Desc`, Count  from DUAL_2G  where Date='$datei' and (`Desc` like 'Number of Dual Handsets in the Network' or `Desc` like  '%Number of Normal Handsets' ) union all select `Desc`, Count  from DUAL_3G  where Date='$datei' and (`Desc` like 'Number of Dual Handsets in the Network' or `Desc` like  '%Number of Normal Handsets' )) X group by `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['counts'];
array_push($v2,"$sup");
...
//print_r($data2y);
if ($datet == $datef){
$title="Number of Handsets Duals vs Normal on $datef";}
else {
$title="Number of Handsets Duals vs Normal between $datef and $datei";
} 
...
$b2plot->SetLegend('Dual Handsets');
$b1plot->SetLegend('Normal Handsets');
