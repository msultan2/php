...
$sql=mysql_query(" select substr(`Desc`,11,4) as `Desc` ,Count from DUAL_3G where Hour=$hour and Date='$date' and (`Desc` like 'Number of Dual Handsets in the Network' or `Desc` like  '%Number of Normal Handsets' )  order by `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['Count'];
