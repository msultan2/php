...
$sql=mysql_query("select trim(substr(`Desc`,28,12)) as `Desc` ,Count from DUAL_3G where Hour=$hour and Date='$date' and `Desc` like 'Number of Dual Handsets in%' and `Desc` not like '%in the Network' and `Desc` not like '%NULL%' order by `Desc`");
while($rows=mysql_fetch_array($sql)):
$sup=$rows['Desc'];
$counts=$rows['Count'];
