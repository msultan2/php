...
$sql=mysql_query("select Hour, substr(`Desc`,32,7) as `Desc`,sum(Count) counts FROM (SELECT Hour,`Desc`, Count FROM DUAL_2G WHERE Date = '$date' and `Desc` like 'Number of Dual Handsets in the Network%'  union All select Hour,`Desc`, Count FROM DUAL_3G WHERE Date = '$date' and `Desc` like 'Number of Dual Handsets in the Network%' ) X GROUP BY `Hour`");
