...
<?php	  
	...
	if (isset($_GET['area'])) {
		...
		$sql = "SELECT DISTINCT BR.Site_ID,DS.Node,TT.Affected_TRUs,DS.DownTime_StartDate,BR.Site_Type,DS.LastModified_Date,BR.Description,Cell.Request_Status,
						HUB,
				...
				FROM dbo.tbl_SS_DownSites DS
				  LEFT OUTER JOIN [SM_Change_Researching_DB].[dbo].[tbl.Bridgs] BR
				  ON  BR.Site_ID = (Select case WHEN DS.[Site_ID] LIKE '%[a-z]%' THEN DS.[Site_ID]
				  ...
				  where BR.Site_ID IS NOT NULL
				  AND BR.Description like '$area'
				  ORDER BY 2,1 ASC;";
...
<script type="text/javascript">       
	...
	function draw_sites() {         
		var dataTable = google.visualization.arrayToDataTable([           
						['Site','Site Type','Code','Node','# Of Affected Cells','Down Date','Number of Cascaded','OSS Last Update','HUB','Remedy TT','SA Date','Assigned Team','Chronic','Outage','P'],                    
			<?php for($i=0;$i<count($data_Desc); $i++) {             
						$site = $data_Desc[$i];
						echo "['".$site."','".$data_siteType[$i]."','".$data_codedConnectivity[$i]."','".$data_siteNode[$i]."','".$data_siteCells[$i]."','".$data_downDate[$i]."',".$Number_of_Cascaded[$i].",'".$data_modifiedDate[$i]."','".$data_HUB[$i]."','".$data_TT[$i]."','".$data_SA_Time[$i]."','".$data_Assigned_Team[$i]."','".$data_Chronic_Site[$i]."','".$data_Outage[$i]."','".$data_Site_Grade[$i]."']"; 
...
<table >
	<tr>
		<td colspan=2><?php echo "$Bridge: $area";?>  </td>
	</tr>
