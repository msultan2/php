...
<?php
	...
	/* Specify the server and connection string attributes. */
		...
		$sql_old = "SELECT SS.Site_ID,DS.cellscount,DS.Urgency,DS.Node,DS.DownTime_StartDate,SS.Site_Type,DS.LastModified_Date,SS.Area,SS.Region,HUB,TTnumber,CAST(SA_Time AS VARCHAR(50)) SA_Time,TT.Assigned_Team,TT.Chronic_Site,TT.Outage,TT.Site_Grade,
		...
		$sql = "SELECT DISTINCT SS.Site_ID,DS.cellscount,DS.Urgency,DS.Node,DS.DownTime_StartDate,SS.Site_Type,DS.LastModified_Date,SS.Area,SS.Region,Cell.Request_Status,
						HUB,
...
<script type="text/javascript">       
	...
	function draw_ChangeTypes() {         
		var dataTable = google.visualization.arrayToDataTable([           
			['Site', 'Site Type','Down Date','Down Days','Requested Status','Number of Cascaded','OSS Last Update','Area','Region','HUB','Remedy TT','SA Date','Assigned Team','Chronic','Outage','P'],                    
			<?php for($i=0;$i<count($data_Desc); $i++) {             
						$site = $data_Desc[$i];
						echo "['".$site."','".$data_siteType[$i]."','".$data_downDate[$i]."',".$data_Val[$i].",'".$data_Halt[$i]."',".$Number_of_Cascaded[$i].",'".$data_modifiedDate[$i]."','".$data_area[$i]."','".$data_region[$i]."','".$data_HUB[$i]."','".$data_TT[$i]."','".$data_SA_Time[$i]."','".$data_Assigned_Team[$i]."','".$data_Chronic_Site[$i]."','".$data_Outage[$i]."','".$data_Site_Grade[$i]."']"; 
