...
['Edfu','Aswan Rural',97,123],
	  ...
	  <?php
		...
		$db_details = "//".$IP.":1521/".$SID;
		echo $db_details;
	  // create connection
		$conn = oci_connect($user,$pass,$db_details);
		...
		echo "Connected :)";
		$query0 =  "select table_name from user_tables where (table_name like ('%WL%') OR table_name like ('%WHIT%') OR table_name like ('%LIST%') )";
						
		$stid0 = oci_parse($conn,$query0);
		oci_execute($stid0);	
		
		echo '</center><h1>White Lists:</h1><center>';
		echo '<table class="blue">';
		echo ' <tr><th>LIST</th></tr>';

		while (oci_fetch($stid0)) {
			$TABLE_NAME = oci_result($stid0, "TABLE_NAME");			
			echo '<tr><td><a href="./WhiteList.php?database='.$database.'&table='.$TABLE_NAME.'">'.$TABLE_NAME.'</a></td></tr>';
		}
		echo '</table>';
