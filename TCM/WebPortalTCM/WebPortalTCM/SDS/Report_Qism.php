...
<?php	  
	...
	if (isset($_GET['Qism_Name'])) {
			
		$Qism_Name = str_replace("__"," ",$_GET['Qism_Name']);
		...
		$sql = "SELECT DISTINCT SS.Site_ID,CC.Code,DS.Node,DS.DownTime_StartDate,SS.Site_Type,DS.LastModified_Date,QR.[Qism NAME] Qism_Name,SS.Region,Cell.Request_Status,
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				array_push($data_Qism_Name,$row['Qism_Name']);
...
<script type="text/javascript">       
	...
	function draw_sites() {         
		var dataTable = google.visualization.arrayToDataTable([           
						['Site','Site Type','Code','Node','# Of Affected Cells','Down Date','Number of Cascaded','Last Update','Qism_Name','Region','HUB','Remedy TT','SA Date','Assigned Team','Chronic','Outage','P'],                    
			<?php for($i=0;$i<count($data_Desc); $i++) {             
						$site = $data_Desc[$i];
						echo "['".$site."','".$data_siteType[$i]."','".$data_codedConnectivity[$i]."','".$data_siteNode[$i]."','".$data_siteCells[$i]."','".$data_downDate[$i]."',".$Number_of_Cascaded[$i].",'".$data_modifiedDate[$i]."','".$data_Qism_Name[$i]."','".$data_region[$i]."','".$data_HUB[$i]."','".$data_TT[$i]."','".$data_SA_Time[$i]."','".$data_Assigned_Team[$i]."','".$data_Chronic_Site[$i]."','".$data_Outage[$i]."','".$data_Site_Grade[$i]."']"; 
