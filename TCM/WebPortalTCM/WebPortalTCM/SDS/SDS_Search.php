	...
	/************************** Excel ***********************/
		...
		echo "<div id='main'>
				...
				<tr>
					...
					<td class='ol-cell col-cell2 col-cellh'>Cascaded Sites</td>
		...
		for ($i=0; $i<sizeof($SiteIDs); $i++){
			...
			$connectionInfo = array( "UID"=>$ini_array['DB_USER'],
				...
				$sql_old = "SELECT DS.Site_ID,DS.cellscount,DS.Node,DS.DownTime_StartDate,SS.Site_Type,DS.LastModified_Date,SS.Area,SS.Region,
						dbo.Cascaded_sites(CC.Code) Cascaded_sites,
				HUB,CASE WHEN TTnumber IS NULL 
				  ...
				  ON  DS.Site_ID = CC.Site_ID
				  WHERE DS.Site_ID = '".$siteID_4d."';";
				  
				$sql = "SELECT SS.Site_ID,DS.cellscount,DS.Node,DS.DownTime_StartDate,SS.Site_Type,DS.LastModified_Date,SS.Area,SS.Region,
						dbo.Cascaded_sites(CC.Code) Cascaded_sites,
				HUB,CASE WHEN TTnumber IS NULL 
							THEN '[RSV] ' + Incident_ID 
							ELSE TTnumber
				  ...
				  ON RSV.SiteID = DS.Site_ID	
				  LEFT OUTER JOIN dbo.tbl_SS_Coded_Connectivity CC
				  ON  SS.Site_ID = (Select case WHEN CC.[Site_ID] LIKE '%[a-z]%' THEN CC.[Site_ID]
													ELSE cast(CAST(CC.[Site_ID] as int) as nvarchar(50))
													END )
				...
				print_r($row);
					...
					$Cascaded_sites = $row['Cascaded_sites'];
					...
					echo "<tr>
						...
						<td class='col-cell col-cell2'><a href=Cascaded_sites.php?site=".$siteID_4d." target=_blank >".$Cascaded_sites."</a></td>
