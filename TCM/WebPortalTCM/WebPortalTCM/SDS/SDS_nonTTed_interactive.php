...
<?php
	...
	/************************** Excel ***********************/
			...
			$connectionInfo = array( "UID"=>$ini_array['DB_USER'],
				...
				$TRU_0='"0"';
				$sql = "SELECT DISTINCT DS.Site_ID,CC.Code,DS.cellscount,DS.Node,CASE TRUCount WHEN '$TRU_0' THEN 0 ELSE TRUCount END TRU_Num,CASE WHEN DS.Urgency like '%TORE%' THEN 'Stores' WHEN DS.Urgency like 'NormaL' THEN 'Normal' ELSE DS.Urgency END Urgency,DS.Node,DS.DownTime_StartDate,SS.Site_Type,DS.LastModified_Date,SS.Area,
...
<script type="text/javascript">       
	...
	function draw_Records() {         
		var dataTable = google.visualization.arrayToDataTable([           
			['Site','Code' ,'Site Type','Node','Down Date','Down Days','Cells','TRUs','Number of Cascaded','Requested Status','OSS Last Update','Area','Region','Remedy TT','SA Date','Parent Site'],                    
			<?php for($i=0;$i<count($data_Desc); $i++) {             
						...
						echo "['".$site."','".$data_Code[$i]."','".$data_siteType[$i]."','".$data_siteNode[$i]."','".$data_downDate[$i]."',".$data_Val[$i].",".$data_siteCells[$i].",".$data_siteTRU[$i].",".$Number_of_Cascaded[$i].",'".$data_Halt[$i]."','".$data_modifiedDate[$i]."','".
