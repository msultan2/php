...
<?php
	if (isset($_GET['type'])) {
		...
		$sql	= "SELECT Map.Governate_Location,Map.Governate_Desc,COUNT(DS.Site_ID) Down,COUNT(SS.Site_ID) AllSites
				  FROM  dbo.tbl_SS_LK_SubRegions_Sites SS
				  LEFT OUTER JOIN dbo.tbl_SS_DownSites DS
				  ...
				  WHERE SS.Site_Type = '$tableType'
				  GROUP BY Map.Governate_Location,Map.Governate_Desc
				  ORDER BY 3 DESC;";
...
?>
	...
	var data = google.visualization.arrayToDataTable([
		// [Region],[color],[size]
		['Governate',  'Description', 'Availability%', 'Down Sites'],
		['Dummy1','Dummy1',0,0],
		//['Dummy2',100,0],
		<?php	for($i=0;$i<count($data_area); $i++) {            
						...
						echo "['".$area."','".
								...
								$data_Val_DownSites[$area]."/".$data_Val_AllSites[$area]."']";
