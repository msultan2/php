...
<script type="text/javascript">       
	...
	function draw_ChangeTypes() {         
		var data = google.visualization.arrayToDataTable([           
			['Region', '2G',  { role: 'annotation' },'3G',  { role: 'annotation' } ],
			<?php for($i=0;$i<count($data_Desc); $i++) {             
						echo "['".$data_Desc[$i]."',".$data_Val_2G[$i].",".$data_Val_2G[$i].",".$data_Val_3G[$i].",".$data_Val_3G[$i]."]"; 
						if ($i<count($data_Desc)-1) echo ",";	//add ',' to all except last element
					}  //&cht=p3
			?>
			] );     
		
		var options = {           title: 'Availability % per Region'  , is3D: true  , chartArea:{left:60,top:20,width:"40%",height:"65%"} };          
