...
<?php
			...
			if ($_GET['color'] === 'Blue') {
				$ColorCondition = "TT.Priority = 'Low' ";
			}
			else if ($_GET['color'] === 'Red') {
				$ColorCondition = "TT.Priority = 'Critical'";
			}
			else if ($_GET['color'] === 'Orange') {
				$ColorCondition = "TT.Priority = 'High'";
			}
			else if ($_GET['color'] === 'Yellow') {
				$ColorCondition = "TT.Priority = 'Medium'";
			}
		}	
		...
		$sql = "SELECT vio.Incident_ID,vio.Assigned_Group,CAST(vio.last_modified_date AS Varchar)last_modified_date,vio.Outage,vio.Status,vio.Region,vio.Sub_Region,vio.Priority,CAST(vio.[Submit Date] AS Varchar)Submit_Date,vio.SiteID,vio.TRUs_Affected,vio.NoOfSites,vio.Outage_Site,TT.Grade,
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) 
		{
			...
			array_push($data_ONoOfSites,$row['NoOfSites']);
...
<script type="text/javascript">       
	...
	function drawAll() {         
		var dataChart = google.visualization.arrayToDataTable([   
				['Incident_ID','Assigned_Group','last_modified_date','Outage','Status','Region','Sub_Region','Priority','Submit_Date','SiteID','TRUs_Affected','NoOfSites','Outage_Site','SLM_Status','Grade'],
				<?php 
				for($i=0;$i<count($data_Incident_ID); $i++) 
				{ 
					echo "['".$data_Incident_ID[$i]."','".$data_Team[$i]."','".$data_last_modified_date[$i]."','".$data_Outage[$i]."','".$data_Status[$i]."','".$data_Region[$i]."','".$data_Sub_Region[$i]."','".$data_Priority[$i]."','".$data_Submit_Date[$i]."','".$data_SiteID[$i]."','".$data_TRUs_Affected[$i]."','".$data_ONoOfSites[$i]."','".$data_Outage_Site[$i]."','".$data_SLM_Status[$i]."','".$data_Grade[$i]."']";
