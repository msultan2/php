<?php
	...
	/* Specify the server and connection string attributes. */
		...
		$sql = " SELECT Assigned_Group,Status,COUNT(vio.Incident_ID) Violated_TT
				  FROM dbo.[vw_SS_Remedy_TT_SLA_Assigned_Violated] vio
				  WHERE Assigned_Group IS NOT NULL
				  AND OUTAGE = 'Yes'
				  group by Assigned_Group,Status
				  order by 3 DESC;";
...
<?php for($i=0;$i<count($data_Desc); $i++) {             
						echo substr($data_Desc[$i],0,15)." ,".$data_Desc_detail[$i]."\t".$data_Val[$i].";";
