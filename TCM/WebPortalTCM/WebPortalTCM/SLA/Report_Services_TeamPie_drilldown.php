<?php
	...
	/* Specify the server and connection string attributes. */
		...
		$sql = " SELECT CASE WHEN Assigned_Group LIKE 'NFM%' THEN 'NFM'
							...
							ELSE Assigned_Group
							END Assigned_Group,CAST(Type AS VARCHAR) Type,COUNT(vio.Incident_ID) Violated_TT
				  FROM dbo.[vw_SS_Remedy_TT_SLA_Assigned_Services_Violated] vio
				  WHERE Assigned_Group IS NOT NULL
				  group by CASE WHEN Assigned_Group LIKE 'NFM%' THEN 'NFM'
							WHEN Assigned_Group LIKE 'Site Manag%' THEN 'Site Management'
							ELSE Assigned_Group
							END,CAST(Type AS VARCHAR)
				  order by 3 DESC;";
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
			if($row['Violated_TT'] > 0){
				...
				array_push($data_Desc_detail,$row['Type']);
...
<pre id="tsv" style="display:none">Team	Type	SR
