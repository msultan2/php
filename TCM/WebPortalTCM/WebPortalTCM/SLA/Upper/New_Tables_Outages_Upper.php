...
<?php
		...
		$sql = "SELECT  CASE WHEN TT.Priority = 'Critical' THEN 'Red'
							WHEN TT.Priority = 'High' THEN 'Orange'
							WHEN TT.Priority = 'Medium' THEN 'Yellow'
							WHEN TT.Priority = 'Low' THEN 'Blue'
							END Severity, 
						CASE WHEN TT.Priority = 'Critical' THEN 1
							WHEN TT.Priority = 'High' THEN 2
							WHEN TT.Priority = 'Medium' THEN 3
							WHEN TT.Priority= 'Low' THEN 4
							END Severity_Order, 
						  ...
						  GROUP BY CASE WHEN TT.Priority = 'Critical' THEN 'Red'
							WHEN TT.Priority = 'High' THEN 'Orange'
							WHEN TT.Priority = 'Medium' THEN 'Yellow'
							WHEN TT.Priority = 'Low' THEN 'Blue'
							END, 
		...
		var dataChart = google.visualization.arrayToDataTable([   
			...
			<?php 
				for($i=0;$i<count($data_Severity); $i++) {             
						...
						echo "['<a href=\"New_Tables_Outages_Links_Upper.php?color=".$data_Severity[$i]."\" target = \'_blank\'>".$data_Severity[$i]."</a>','".$data_Violated[$i]."','".$data_Total[$i]."']"; 
