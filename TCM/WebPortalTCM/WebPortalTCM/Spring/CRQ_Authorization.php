	...
	if (isset($_POST['CRQnums'])) {
		...
		if( !$conn ) {
			 die( print_r( sqlsrv_errors(), true));
				...
				AND Assignee_Group not like 'Access%Planned%Activities';";
...
<?php
		...
		$connectionInfo = array( "UID"=>$ini_array['DB_USER'],
			...
			$sql = "SELECT DISTINCT  ordr.Team_Order,cab.Impact,cab.Risk_Level,cab.CRQ,Requester,Support_Company,Support_Organization,Support_Group_Name,Description,Status,Assignee_Company , Assignee_Organization, Assignee_Group,
			...
			while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				echo "<td class=Major><U>CRQ: ".substr($CRQ, -6)."</U><BR>".$Description."<B><BR><U>Impact: </U>".$Impact."<BR><U>Risk: </U>".$Risk."<B>";
...
<table width="100%"><tr>
	<td>
		<table><tr><td><div class="body_text"><b>Authorized Changes</b></div></td>
		<td align=right >
		<?php 
				$sql = "select cab.CRQ,Description,Justification,Support_Organization,Assignee_Group,Status,Impact,cab.DWH_Impact,External_Customer,Internal_Customer,cab.Nodes_Systems from dbo.vw_Change_Daily_CAB cab,dbo.tbl_Change_Daily_CAB_Authorize auth where cab.CRQ = auth.CRQ  And Authorized = '1';";
