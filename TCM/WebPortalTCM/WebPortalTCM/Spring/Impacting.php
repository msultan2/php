...
<?php
		...
		$connectionInfo = array( "UID"=>$ini_array['DB_USER'],
			...
			$sql = "SELECT ch.CRQ,Requester,Description,Support_Group_Name,Assignee_Group,Scheduled_Start_Date,Scheduled_End_Date,Impact,External_Customer,Service_Impact,Internal_Customer,Business_Impact,Nodes_Systems,Technical_Impact,Reporting,DWH_Impact
				FROM dbo.vw_Change_Daily_CAB ch  
				WHERE ( External_Customer NOT IN ('na','NA','N/A') OR  Service_Impact NOT IN ('na','NA','N/A') OR  Internal_Customer NOT IN ('na','NA','N/A') OR  Business_Impact NOT IN ('na','NA','N/A') OR  Nodes_Systems NOT IN ('na','NA','N/A') OR  Technical_Impact NOT IN ('na','NA','N/A') OR  Reporting NOT IN ('na','NA','N/A') OR  DWH_Impact NOT IN ('na','NA','N/A') ) 
			...
			while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				echo "<td>".$Assignee_Group."</td>";
				echo "<td class=Major><U>CRQ: ".$CRQ."</U><BR>".$Description."<B>";
