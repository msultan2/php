...
<table><tr>
	...
	<tr><td><textarea name="query" cols="125" rows="25">
		<?php 
			$sql = "SELECT DISTINCT cab.CRQ 
					FROM dbo.vw_Change_Daily_CAB cab,dbo.tbl_Change_Daily_CAB_Authorize auth
					WHERE cab.CRQ = auth.CRQ AND auth.Authorized = 1
			...
			while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				if ($count == 0) 	
					$strCRQ = "'Status*' = ".'"Request For Authorization"'. " AND ( 'Change ID*+' = ".'"'. $row['CRQ'] .'" ';
				else				
