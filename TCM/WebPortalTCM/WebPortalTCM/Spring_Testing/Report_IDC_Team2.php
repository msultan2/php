	...
	/* Specify the server and connection string attributes. */
		...
		$sql = "SELECT CASE WHEN inc.Owner_Team = 'Other'
						...
						ON inc.Owner_Team = team_map.INC_OwnerTeam
								WHERE (dbo.DateOnly(inc.[Start Date]) <= '$getTo' AND dbo.DateOnly(inc.[Start Date]) >= '$getFrom')
								AND Dueto_Change = 1
...
<script type="text/javascript">       
	...
	function draw_ChangeFor(){
		...
		//drawPieChart
		var options = {           title: 'IDCs Teams'  , is3D: true  , chartArea:{left:20,top:20,width:"60%",height:"90%"} };          
