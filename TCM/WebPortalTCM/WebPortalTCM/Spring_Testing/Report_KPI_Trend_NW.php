...
<script type="text/javascript">       
	...
	function drawAll() {         
		var dataTable = google.visualization.arrayToDataTable([           
			['Month', 'Authorized','Standard','Normal','Exceptions','Exceptions%','Fix','Fix%','Off Process','Off%','Unauthorized','Unauthorized','Adhoc','Adhoc%','IDC','IDC%'],  
			<?php	for($i=0;$i<count($data_month); $i++) {            
						...
						elseif ($data_Val_INC_DueChange[$month] == '') 
							...
							//	round(($data_Val_unauth[$month]/($data_Val_unauth[$month]+$data_Val_Normal_After[$month]))*100,0)."\%',".
								round(($data_Val_unauth[$month]/($data_Val_unauth[$month]+$data_Val_After[$month]))*100,0)."\%',".
								$data_Val_Late[$month].",'".
