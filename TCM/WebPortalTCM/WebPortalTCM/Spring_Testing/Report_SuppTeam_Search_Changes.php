...
<?php // content="text/plain; charset=utf-8"
	...
	/* Specify the server and connection string attributes. */
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {	
			...
			array_push($data_ApPending,$row['APApprovers']);
...
<script type="text/javascript">       
	...
	function drawAll() {         
		var dataTable = google.visualization.arrayToDataTable([   
				['CRQ','CRQ_Type','Description','Support_Company','Support_Organization','Support_Group_Name','Assignee_Group','Scheduled_Start_Date','Approval Pending','Last Modified By','Status'],
				
		<?php 
				for($i=0;$i<count($data_CRQ); $i++) 
				{ 
					echo "['".$data_CRQ[$i]."','".$data_CRQ_Type[$i]."','".$data_Description[$i]."','".$data_Support_Company[$i]."','".$data_Support_Organization[$i]."','".$data_Support_Group_Name[$i]."','".$data_Assignee_Group[$i]."','".$data_Scheduled_Start_Date[$i]."','".$data_ApPending[$i]."','".$data_Last_Modified_By[$i]."','".$data_Status[$i]."']";
