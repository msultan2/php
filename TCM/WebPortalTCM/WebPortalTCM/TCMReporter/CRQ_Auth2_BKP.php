...
<?php
		...
		$connectionInfo = array( "UID"=>$ini_array['DB_USER'],
			...
			while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				$Impact = $Impact_Exp['impact_level'];
				$is_Auth=($Authorized == 1)? 'checked':'';
				if($row['External_Impact']!="") $Impact_Service=$row['External_Impact'].": ".$row['Service_Impact'];
				...
				echo "<tr><td>".$index."</td>
					<td>".substr($CRQ, -6)."</td>
					<td>".$Submitter."</td>
					<td width=140px ><table><tr><th>".$SupportCompany."</th></tr><tr><td>".$SupportOrganization."</td></tr><tr><td>".$SupportGroup."</td></tr></table></td>";
				echo ($Assignee_Company == null)?"<td></td>":"<td width=140px><table><tr><th>".$Assignee_Company."</th></tr><tr><td>".$Assignee_Organization."</td></tr><tr><td>".$Assignee_Group."</td></tr></table></td>";
				echo	"<td width=200px>".$ScheduledStartDate."<br> to <br>".$ScheduledEndDate."</td>
					<td class=Major>".$Description."<B>";
				...
				echo "</B></td>";
					//<td class=".$Impact.">".$Impact."</td>
					//<td>". ( ($Status==="Scheduled")? '--':$PendingAt ) . "</td>
				echo "<td class='".$ChangeFor."'>".$ChangeFor."<table width=100%><tr><th>".$ProductTier1."</th></tr><tr><td>".$ProductTier2."</td></tr><tr><td>".$ProductTier3."</td></tr></table></td>
					<td>".$Status."</td>";
				?>
