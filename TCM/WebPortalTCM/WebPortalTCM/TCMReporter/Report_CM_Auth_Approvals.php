...
require_once ('jpgraph/jpgraph_bar.php');
		...
		$j=1;
		for ($i=0; $i < 3; $i++) { 
			$sql = "SELECT CM_Auth_Approver,count(*) CRQnum
					FROM (
						SELECT 
							CASE 
	...
	$graph->title->Set("CM Authorization Approvals");
	...
	//$graph->legend->Pos(0.05,0.5,'center','bottom');
	...
	$graph->Stroke();
		...
		for ($i=0; $i < 3; $i++) { 
			$sql = "WITH Temp_CM_Approvals AS (
					...
					SELECT schYear, schMonth,CM_Auth_Approver,count(*) CRQnum
					FROM Temp_CM_Approvals
					GROUP BY schYear, schMonth,CM_Auth_Approver
					ORDER  BY schYear DESC, schMonth DESC,CM_Auth_Approver;";
					//echo " ss=".$sql;
			$stmt = sqlsrv_query( $conn, $sql );
			if( $stmt === false) {
				die( print_r( sqlsrv_errors(), true) );
