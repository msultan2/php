...
<table><tr><td width="100%"><div class="body_text"><B>KPI Monthly Trend Summary</B></div></td>
<td align=right >
<?php 
	$startDate = '4/1/2013';
...
<?php // content="text/plain; charset=utf-8"
	...
	/* Specify the server and connection string attributes. */
		...
		$data_Val_AdhocNR = array();
		$data_Val_AdhocST = array();
		...
		$sql = "SELECT YEAR(ch.Scheduled_Start_Date) year_,MONTH(ch.Scheduled_Start_Date) month_,count(*) CRQnum
				...
				WHERE (Scheduled_Start_Date >= '8/1/2012' AND Scheduled_Start_Date <= GETDATE())
				AND (dbo.DateOnly(ch.Scheduled_Start_Date) <= '$getTo' AND dbo.DateOnly(ch.Scheduled_Start_Date) >= '$getFrom')
		...
		$sql_old = "SELECT YEAR(ch.Scheduled_Start_Date) Scheduled_year,MONTH(ch.Scheduled_Start_Date) AS Scheduled_month,
				...
				isnull(count(Adhoc.CRQ ),0) 'Late Submitted'
				FROM  dbo.vw_Change_Approval_Details ch
				LEFT OUTER JOIN (SELECT * FROM  dbo.vw_Change_Approval_Details where CRQ_Type = 'Standard' AND Status != 'Request For Authorization' ) Standar ON ch.CRQ = Standar.CRQ 
				LEFT OUTER JOIN (SELECT * FROM  dbo.vw_Change_Approval_Details where CRQ_Type = 'Normal' AND Status != 'Request For Authorization') Normal ON ch.CRQ = Normal.CRQ 
				...
				LEFT OUTER JOIN (SELECT * FROM  dbo.vw_Change_Approval_Details where Status NOT IN ('Request For Authorization','Cancelled','Rejected') AND dbo.DateOnly(Submit_Date) >= dbo.DateOnly(dbo.udf_GetPrevNextWorkDay(Scheduled_Start_Date,'Previous'))) Adhoc ON ch.CRQ = Adhoc.CRQ 
		...
		$sql = "SELECT YEAR(ch.Scheduled_Start_Date) Scheduled_year,MONTH(ch.Scheduled_Start_Date) AS Scheduled_month,
				...
				isnull(count(Excep.CRQ ),0) 'Exceptions', isnull(count(Fix.CRQ ),0) 'Fix',
				isnull(count(Unauth.CRQ ),0) 'Unauthorized', isnull(count(Off_CM.CRQ ),0) 'Off CM Process', 
				isnull(count(AdhocNR.CRQ ),0) 'Adhoc NR',
				isnull(count(AdhocST.CRQ ),0) 'Adhoc ST'
				FROM  dbo.vw_Change_Approval_Details ch
				...
				LEFT OUTER JOIN (SELECT * FROM  dbo.vw_Change_Approval_Details where [First Approval Date] IS NULL AND Status NOT IN ('Request For Authorization','Cancelled') AND ChangeFor <> 'General') Off_CM ON ch.CRQ = Off_CM.CRQ 
				LEFT OUTER JOIN (SELECT * FROM  dbo.vw_Change_Approval_Details where CRQ_Type = 'Normal' AND Status NOT IN ('Request For Authorization','Cancelled','Rejected') AND dbo.DateOnly(Submit_Date) >= dbo.DateOnly(dbo.udf_GetPrevNextWorkDay(Scheduled_Start_Date,'Previous'))) AdhocNR ON ch.CRQ = AdhocNR.CRQ 
				LEFT OUTER JOIN (SELECT * FROM  dbo.vw_Change_Approval_Details where CRQ_Type = 'Standard' AND Status NOT IN ('Request For Authorization','Cancelled','Rejected') AND dbo.DateOnly(Submit_Date) >= dbo.DateOnly(dbo.udf_GetPrevNextWorkDay(Scheduled_Start_Date,'Previous'))) AdhocST ON ch.CRQ = AdhocST.CRQ 
				WHERE ch.Scheduled_Start_Date <= GETDATE()
				AND ( dbo.DateOnly(ch.Scheduled_Start_Date) <= '$getTo' AND dbo.DateOnly(ch.Scheduled_Start_Date) >= '$getFrom')
				--AND ch.Support_Company NOT IN ('Products & Services Delivery','IT Operations')
				GROUP BY YEAR(ch.Scheduled_Start_Date),MONTH(ch.Scheduled_Start_Date)
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
			...
			$data_Val_AdhocNR[$date] = $row['Adhoc NR'];
			$data_Val_AdhocST[$date] = $row['Adhoc ST'];
		}
		sqlsrv_free_stmt( $stmt);
...
<script type="text/javascript">       
	...
	function drawAll() {         
		var dataTable = google.visualization.arrayToDataTable([   
			['Month', 'Authorized','Standard','Normal','Exceptions','Exceptions%','Fix','Fix%','Off Process','Off%','Unauthorized NR','Unauthorized NR%','Adhoc NR','Adhoc NR%','Adhoc ST','Adhoc ST%','IDC','IDC%'],  
			<?php	for($i=0;$i<count($data_month); $i++) {            
						...
						elseif ($data_Val_INC_DueChange[$month] == '') 
							echo "['".$data_month[$i]."',".
								...
								$data_Val_AdhocNR[$month].",'".
								round(($data_Val_AdhocNR[$month]/$data_Val_Normal_After[$month])*100,0)."\%',".
								$data_Val_AdhocST[$month].",'".
								round(($data_Val_AdhocST[$month]/$data_Val_Standard[$month])*100,0)."\%',".
								$data_Val_INC_DueChange[$month].",' ']";
