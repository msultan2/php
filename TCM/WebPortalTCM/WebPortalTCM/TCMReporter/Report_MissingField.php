...
<table><tr><td width="90%"><div class="body_text">Requesters of Changes with Missing Fields (since 12 Dec 2012)</div></td>
<td align=left >
<?php 
		...
		$sql = "select ch.Requester,ch.Support_Company,ch.Support_Organization,ch.Support_Group_Name,count(mf.CRQ) 'CRQs with Missing Fields',count(ch.CRQ) 'Total Normal CRQs' FROM  dbo.vw_Change_Approval_Details ch LEFT OUTER JOIN (SELECT * FROM  dbo.vw_Change_Approval_Details WHERE Missing_Fields = 0) mf ON ch.CRQ = mf.CRQ WHERE ch.CRQ_Type = 'Normal' AND ch.Scheduled_Start_Date > '12/12/2012' group by ch.Requester, ch.Support_Company, ch.Support_Organization, ch.Support_Group_Name order by 5 DESC;";
...
<?php
		...
		$sql = "SELECT cap.Requester, CASE cap.Support_Company
						...
						WHEN 'IT Operations' THEN 'ITOP'
						ELSE 'OTHER' 
					END supp_company,cap.Support_Organization,cap.Support_Group_Name, count(mf.CRQ) CRQnum, count(cap.CRQ) Total
							FROM  dbo.vw_Change_Approval_Details cap LEFT OUTER JOIN 
						(SELECT * FROM  dbo.vw_Change_Approval_Details WHERE Missing_Fields = 0) mf ON cap.CRQ = mf.CRQ 					
				  WHERE cap.CRQ_Type = 'Normal' --AND cap.Status != 'Request For Authorization'
				  AND cap.Scheduled_Start_Date > '12/12/2012'
...
<script type="text/javascript">       
	...
	function drawAll() {         
		var dataTable = google.visualization.arrayToDataTable([           
			['Requester', 'Dept','Team', 'CRQs with Missing Fields','Total Normal Changes'],           
			<?php 
				//echo "[";
		...
		//drawPieChart
		var options = { title: 'CRQ Requesters with Missing Fields more than twice'  ,  chartArea:{left:40,top:40,width:"55%",height:"80%"} //legendTextStyle: {color:'#00FF00'}, //vAxis: {maxValue:3, minValue:1}
