...
<script type="text/javascript">       
	...
	function draw_ChangeFor(){
		var data = google.visualization.arrayToDataTable([           
			['Changes For', 'Authorized Changes'],           
			<?php for($i=0;$i<count($data_Desc); $i++) {             
						echo "['".$data_Desc[$i]." Standard Changes',".$data_Val_ST[$i]."],"; 
						echo "['".$data_Desc[$i]." Non-Standard Changes',".$data_Val_NR[$i]."]"; 
						if ($i<count($data_Desc)-1) echo ",";	//add ',' to all except last element
					}  //&cht=p3
			?>	] );       
		var options = { title: 'CRQs Authorized per Domain', is3D: true  , chartArea:{left:40,top:30,width:"90%",height:"90%"} ,
						slices:{3:{offset: 0.2} ,2: {offset: 0.2} }, pieSliceTextStyle:{fontSize:12} ,colors:["#B47AFF","#F67AFF","#7AFF9E","#C6FF7A"] };          
