...
<script type="text/javascript">       
	...
	function drawAll() {         
		...
		var dataChart_new = google.visualization.arrayToDataTable([           
			...
			<?php 
				...
				foreach ($data_Date as &$month) {             
					//if ($i==count($data_Date)-1) break;		//ignore current month
					if ($i<count($data_Date)-6 ){ $i++; continue;	}	//ignore before April 2013
					echo "['".$month."',".$data_Val_Normal[$month].",".round(($data_Val_Emergency[$month] / $data_Val_Normal[$month])*100)." ]";
					if ($i<count($data_Date)-1) echo ",";	//add ',' to all except last element
					$i=$i+1;
				}
			?>
		...
		var options_temp = { title: 'NW Emergency Changes Trend'  ,  chartArea:{left:40,top:40,width:"75%",height:"65%"},vAxis: {gridlines:{count:8}}, //maxValue:3, minValue:1},
						hAxis: {title: 'Month', gridlines:{count:6}, slantedText:true}  ,colors:['darkred'], legend:{position:'none'}, selectionMode:'multiple'};          
