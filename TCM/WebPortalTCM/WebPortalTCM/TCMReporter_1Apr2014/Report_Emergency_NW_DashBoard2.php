...
<?php // content="text/plain; charset=utf-8"
		...
		for ($i=0; $i < $ini_array['NUM_OF_MONTHS']; $i++) {
			...
			$sql = "SELECT YEAR(Scheduled_Start_Date) Scheduled_year,MONTH(Scheduled_Start_Date) AS Scheduled_month,isnull(count(*),0) CRQnum
				...
				AND Status NOT IN ('Draft', 'Request For Authorization')
				AND CRQ_Type = 'Normal'
				AND NOT ".$ini_array['IT_CONDITION']."
				AND [First Approval Date] IS NOT NULL
	...
	function drawAll() {         
		...
		var dataChart_temp = google.visualization.arrayToDataTable([           
			...
			<?php 
				...
				foreach ($data_Date as &$month) {             
					//if ($i==count($data_Date)-1) break;		//ignore current month
					if ($i<count($data_Date)-6 ){ $i++; continue;	}	//ignore before April 2013
		...
		var dataChart_new = google.visualization.arrayToDataTable([           
			['Month of Year','Non-Standard','Exceptions'],           
			<?php 
				$i=0;
				foreach ($data_Date as &$month) {             
					//if ($i==count($data_Date)-1) break;		//ignore current month
					if ($i<count($data_Date)-6 ){ $i++; continue;	}	//ignore before April 2013
					echo "['".$month."',".($data_Val_Normal[$month]-$data_Val_Emergency[$month]).",".$data_Val_Emergency[$month]." ]";
					if ($i<count($data_Date)-1) echo ",";	//add ',' to all except last element
					$i=$i+1;
				}
			?>
		...
		var options_temp = { title: 'NW Emergency Changes Trend'  ,  chartArea:{left:40,top:40,width:"75%",height:"65%"},vAxis: {gridlines:{count:8}}, //maxValue:3, minValue:1},
						hAxis: {title: 'Month', gridlines:{count:6}, slantedText:true} , isStacked:true, legend:{position:'bottom'}, selectionMode:'multiple'};          
		var chart = new google.visualization.ColumnChart(document.getElementById('chart_div'));         
		chart.draw(dataChart_new, options_temp);       
		//chart.draw(dataChart, options);       
	} 
</script>   
<table >
	<tr><td class="iframe_td"><div id="chart_div" style="width: 350px; height: 300px;"></div>  </td></tr>
	<tr><td> <a href="Reporter_Emergency.php" style="margin-left: 20px" target="_blank">More..</a></td></tr>
</table>
