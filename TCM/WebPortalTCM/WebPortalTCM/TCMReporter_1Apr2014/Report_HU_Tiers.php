...
<?php
		...
		$sql="SELECT count(*) CRQnum FROM  dbo.vw_Change_Approval_Details WHERE Scheduled_Start_Date <= getdate() AND CRQ_Type = 'Normal';";
		$stmt = sqlsrv_query( $conn, $sql );
		$data_total = 0;
...
<script type="text/javascript">       
	...
	function drawAll() {         
		...
		var options = { title: 'Top 10 Change Tiers: <?php echo $top10_percent;?>%'  ,  chartArea:{left:200,top:40,bottom:20,width:"75%",height:"80%"}, colors:['darkgreen'], //vAxis: {maxValue:3, minValue:1},
						hAxis: {title: 'High Usage Change Tiers for Normal Changes', titleTextStyle: {color: 'red'}, gridlines:{count:7}}  };          
