...
<table><tr><td width="90%"><div class="body_text">Number of Normal Changes & Incidents per Month since 1st Jan 2012 </div></td>
<td align=right >
<?php 
...
<?php // content="text/plain; charset=utf-8"
	...
	/* Specify the server and connection string attributes. */
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
			...
			$m = $row['month_'];
			$data_month_year = substr(date("F", strtotime("-$m month")),0,3).' '.date("Y", strtotime("-$m month"));
			//$date = substr(date("F", strtotime("-$m month")),0,3).' '.date("Y", strtotime("-$m month"));
			if($date != '8-2012')
	...
	function drawAll() {         
		...
		var dataChart_All = google.visualization.arrayToDataTable([           
			/*	['Week', 'Number of Changes'],
				...
				['12 2012',5,4,3,1,2,6],
			*/
			['Month', 'Before Process Changes','After Process Changes','Normal Authorized Changes','Number of Incidents','Incidents due to Changes'], 
			<?php	for($i=0;$i<count($data_month); $i++) {     
						$month = $data_month[$i];
...
var options = { title : 'Trend of Incidents due to Changes',     
				...
				hAxis: {title: "Month"},    
				//curveType: "function" ,
				seriesType: "area",    'isStacked': false , 
...
<table >
	<tr><td class="iframe_td"><div id="chart_div" style="width: 1400px; height: 400px;"></div>  </td></tr>
