...
<?php
		...
		$sql = "SELECT approver,[Before CAB day] 'Before CAB day',[Before 5 PM] 'Before 5 PM',[After CAB meeting] 'After CAB meeting',[On same Scheduled Day] 'On same Scheduled Day',[After Scheduled Date] 'After Scheduled Date'
				  FROM (
				  SELECT approver,
				  CASE WHEN dbo.DATEONLY( ap.AppDateTime) < (dbo.DATEONLY( Scheduled_Start_Date) -1) THEN 'Before CAB day'
						WHEN DATEADD(day, -DATEDIFF(day, 0, ap.AppDateTime), ap.AppDateTime) <= '5:00:00 PM' AND (ap.AppDateTime <  Scheduled_Start_Date) THEN 'Before 5 PM'
						WHEN DATEADD(day, -DATEDIFF(day, 0, ap.AppDateTime), ap.AppDateTime) > '5:00:00 PM' AND (dbo.DATEONLY( ap.AppDateTime) = (dbo.DATEONLY( Scheduled_Start_Date) -1)) THEN 'After CAB meeting'
				  ...
				  END hourInterval--,ap.AppDateTime,Scheduled_Start_Date,ch.CRQ --
					...
					where Approval_On_Behalf = 'CM_Authorized'
					...
					group by Approver,
						  ...
						  END 
							...
							PIVOT 
								( max(CRQnum)
									for hourInterval in ([Before CAB day],[Before 5 PM],[After CAB meeting] ,[On same Scheduled Day],[After Scheduled Date])
								) queryP  ";
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
			...
			array_push($data_Desc[1],$row['Before 5 PM']);
...
<script type="text/javascript">       
	...
	function drawAll() {         
		var dataTable = google.visualization.arrayToDataTable([           
			['Approver', 'Before CAB day','Before 5 PM','After CAB meeting' ,'On same Scheduled Day','After Scheduled Date','Total','Auth \%'],           
			<?php 
		...
		var dataChart = google.visualization.arrayToDataTable([   
			['Approver', 'Before CAB day','Before 5 PM','After CAB meeting' ,'On same Scheduled Day','After Scheduled Date'],           
			<?php 
				for($i=0;$i<count($data_approver); $i++) {             
						echo "['".$data_approver[$i]."',".$data_Desc[0][$i].",".$data_Desc[1][$i].",".$data_Desc[2][$i].",".$data_Desc[3][$i].",".$data_Desc[4][$i]."]"; 
						if ($i<count($data_approver)-1) echo ",";	//add ',' to all except last element
		...
		//drawPieChart
		var options = { title: 'CM Authorization Approvals Analysis'  ,  chartArea:{left:60,top:40,bottom:120,width:"75%",height:"65%"}, 
