...
<?php
	$startback = date('m/d/Y', time() + (60 * 60 * 24 * -30));
	$yesterday = date('m/d/Y', time() + (60 * 60 * 24 * -1));
	//echo $weekback." ".$yesterday;

	if(isset($_GET['from'])) $getFrom = $_GET['from']; else $getFrom = $startback;
	...
	/* Parse configuration file */
		...
		$sql = "SELECT DISTINCT  ch.CRQ,Requester,ch.Support_Company,ch.Support_Organization,ch.Support_Group_Name,Description,Status,
							Assignee_Company , Assignee_Organization, Assignee_Group,Risk_Level,Impact,dbo.dateOnly(ch.Scheduled_Start_Date) startDate,
							CAST(ch.Scheduled_Start_Date AS varchar) Scheduled_Start_Date,CAST(ch.Scheduled_End_Date AS varchar) Scheduled_End_Date,
							ChangeFor,CAST(ch.cTimeStamp AS varchar) cTimeStamp,
				...
				WHERE dbo.DateOnly(ch.Scheduled_Start_Date) BETWEEN '$getFrom' AND '$getTo'
					AND (SUBSTRING(Impact, 1, 1) IN ('1','2','3') OR SUBSTRING(Risk_Level, 12,1) IN ('3','4','5') )
					AND Status NOT IN ('Request For Authorization','Cancelled','Rejected','Pending')
					AND CRQ_Type = 'Normal'
				ORDER BY  dbo.dateOnly(ch.Scheduled_Start_Date),Assignee_Group,ch.CRQ;";
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				$data_Data[$i][RiskLevel]=$RiskLevel; 
...
<script type="text/javascript">       
	...
	function drawAll() {         
		...
		//	,'SupportCompany','SupportOrganization', 'Assignee_Company','Assignee_Organization',
			...
			'Change Tiers','Status','Scheduled Date','Risk Level','Impact'],                
			<?php 
				...
				//		$data_Data[$i][SupportCompany]."','".$data_Data[$i][SupportOrganization].
						echo "['".$data_Data[$i][CRQ]."','".$data_Data[$i][Submitter]."','".$data_Data[$i][SupportGroup]."','".$data_Data[$i][Assignee_Group].
							 ...
							 " > ".$data_Data[$i][ProductTier3]."','".$data_Data[$i][Status]."','".$data_Data[$i][ScheduledStartDate]."','".$data_Data[$i][RiskLevel].
							 "','".$data_Data[$i][Impact]."']"; 
...
<table width="100%"><tr>
	<td>
		<table><tr><td><div class="body_text"><b>Major Activities</b></div></td>
		<td align=right >
		<?php 
