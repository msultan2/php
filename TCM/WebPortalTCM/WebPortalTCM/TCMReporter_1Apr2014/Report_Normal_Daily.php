...
<?php // content="text/plain; charset=utf-8"
	...
	/* Specify the server and connection string attributes. */
		...
		//Normal
		$sql = "SELECT DATENAME(dw,ch.Scheduled_Start_Date) weekDay , CONVERT(VARCHAR(10), dbo.dateOnly(ch.Scheduled_Start_Date), 103) 'myDay',dbo.dateOnly(ch.Scheduled_Start_Date),count(*) 'Normal',count(emergency_cap.CRQ) 'Exceptions'
				FROM dbo.tbl_Change_Approval_Details ch LEFT OUTER JOIN 
								  (SELECT * FROM  dbo.vw_Change_Approval_Details 
											...
											AND ( (DATEADD(day, -DATEDIFF(day, 0, [First Approval Date]), [First Approval Date]) > '2:00:00 PM'
												...
												OR	Emergency = 0 ) ) emergency_cap ON ch.CRQ = emergency_cap.CRQ 
				WHERE dbo.dateOnly(ch.Scheduled_Start_Date) BETWEEN dbo.dateOnly(getdate()) -30 AND dbo.dateOnly(getdate())
				AND ch.CRQ_Type = 'Normal'
				AND ch.STATUS != 'Request For Authorization'
				GROUP BY DATENAME(dw,ch.Scheduled_Start_Date),CONVERT(VARCHAR(10), dbo.dateOnly(ch.Scheduled_Start_Date), 103),dbo.dateOnly(ch.Scheduled_Start_Date)
				ORDER BY dbo.dateOnly(ch.Scheduled_Start_Date)";
