		...
		<table>
			<form action="CRQ_Auth.php" name="myForm" method="post">
				<tr>
					...
					<td align=left>
						<textarea wrap="hard" rows="08" cols="13" name="CRQnums"><?php echo $_POST['CRQnums'];?></textarea>
					</td>
					<td valign=top>
	...
	if (isset($_POST['CRQnums'])) {
		...
		for ($i=0; $i<sizeof($CRQnums); $i++){
			...
			if(strlen($CRQnum)!=5 OR !is_numeric($CRQnum)){
				...
				$sql = "SELECT cab.CRQ,Requester,Support_Company,Support_Organization,Support_Group_Name,Assignee_Company , Assignee_Organization, Assignee_Group,
...
<?php
			...
			$sql = "SELECT cab.CRQ,Requester,Support_Company,Support_Organization,Support_Group_Name,Description,Status,Assignee_Company , Assignee_Organization, Assignee_Group,
			...
			while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
				...
				<?php
				echo "<td><textarea name='new_comment_".$CRQ."' rows=4 cols=20>".$CM_Comment."</textarea> </td>";
