			...
			<form action="CRQ_Search.php" name="myForm" method="post">
				<tr>
					...
					<td align=left valign=bottom>
						<textarea wrap="hard" rows="08" cols="15" name="CRQnums"><?php echo $_POST['CRQnums'];?></textarea>
					</td>
					<td valign=top>
...
if (isset($_POST['CRQnums'])) {	
	...
	for ($i=0; $i<sizeof($CRQnums); $i++){
		...
		if((strlen($CRQnum) >6 AND substr($CRQnum, 0,3)!= 'CRQ' ) OR strlen($CRQnum) <5 ){
			echo "<tr><td>".$CRQnum."</td><td colspan=12>Invalid CRQ number</td></tr>";
		}
		else{
		...
		$connectionInfo = array( "UID"=>$ini_array['DB_USER'],
			...
			elseif(strlen($CRQnum) == 5) $CRQnumber="CRQ0000000".$CRQnum;
			elseif(substr($CRQnum, 0,3)== 'CRQ') $CRQnumber=$CRQnum;
			//echo substr($CRQnum, 0,3);
			if(isset($CRQnumber)){
				if(substr($CRQnum, -6,1)== '2')
				$sql = "SELECT DISTINCT ch.CRQ,Requester,ch.Support_Company,ch.Support_Organization,ch.Support_Group_Name,tsk.Assignee_Company , tsk.Assignee_Organization, tsk.Assignee_Group,
							...
							Product_Categorization_Tier_1,Product_Categorization_Tier_2,Product_Categorization_Tier_3, emergency,
						...
						FROM dbo.vw_Change_Approval_Details_Ver7_6 ch LEFT OUTER JOIN  dbo.tbl_Change_Task_Merged_Ver7_6 tsk
						ON ch.CRQ = tsk.Request_ID 
						WHERE ch.CRQ = '".$CRQnumber."';";
				else
				$sql = "SELECT DISTINCT ch.CRQ,Requester,ch.Support_Company,ch.Support_Organization,ch.Support_Group_Name,tsk.Assignee_Company , tsk.Assignee_Organization, tsk.Assignee_Group,
							Description,Status, CAST(ch.Scheduled_Start_Date AS varchar) Scheduled_Start_Date,CAST(ch.Scheduled_End_Date AS varchar) Scheduled_End_Date,Impact ,APApprovers, ChangeFor,
							Product_Categorization_Tier_1,Product_Categorization_Tier_2,Product_Categorization_Tier_3, emergency,
				...
				while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
					...
					echo ($Exception==0)?	'<BR><font color=red><B>Exception</B></font>':'';
					echo "</td>
						<td>".$Submitter."</td>
				...
				if(!$found)
						echo "<tr><td>".$CRQnum."</td><td colspan=12>Not found in the Database</td></tr>";
			}					
		}
	}
	echo "</table>";
