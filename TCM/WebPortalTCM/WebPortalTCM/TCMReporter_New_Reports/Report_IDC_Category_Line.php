...
<?php // content="text/plain; charset=utf-8"
		...
		foreach ($array_category as &$category) {
			...
			for ($i=0; $i < $ini_array['NUM_OF_MONTHS_IDC']; $i++) {
				$sql_older = "SELECT YEAR(cap.Scheduled_Start_Date) Scheduled_year,MONTH(cap.Scheduled_Start_Date) AS Scheduled_month,Status ,count(*) CRQnum
				...
				$sql_old = "SELECT YEAR(inc.[Start Date]) incYear,MONTH(inc.[Start Date]) incMonth, count(*) INCnum
				...
				$sql = "SELECT YEAR(inc.[Start Date]) incYear,MONTH(inc.[Start Date]) incMonth, count(*) INCnum
					FROM dbo.tbl_incident_IDC inc LEFT OUTER JOIN dbo.tbl_Incident_LK_Team_Mapping team_map
					ON inc.Owner_Team = team_map.INC_OwnerTeam ";
