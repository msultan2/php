...
<?php // content="text/plain; charset=utf-8"
	...
	/* Specify the server and connection string attributes. */
		...
		$data_month_Standard_No_CM = array();
		$data_Val_INC = array();
		...
		// Standard with no CM CRQs After Process
		$sql = "SELECT YEAR(ch.Scheduled_Start_Date) year_,MONTH(ch.Scheduled_Start_Date) month_,count(*) CRQnum
				FROM dbo.tbl_Change_Approval_Details ch
				WHERE dbo.dateOnly(Scheduled_Start_Date) BETWEEN dbo.dateOnly('8/12/2012') AND dbo.dateOnly(getdate())
				AND Status NOT IN ('Draft','Request For Authorization','Rejected') 
				AND CRQ_Type = 'Standard'
				AND [First Approval Date] IS NULL
				AND Product_Categorization_Tier_1 <> 'Fixed Network Installation'
				GROUP BY YEAR(ch.Scheduled_Start_Date),MONTH(ch.Scheduled_Start_Date)
				ORDER BY YEAR(ch.Scheduled_Start_Date),MONTH(ch.Scheduled_Start_Date);";
		$stmt = sqlsrv_query( $conn, $sql );
		//echo $sql;
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
			...
			//$date = substr(date("F", strtotime("$m month")),0,3).' '.$row['year_'];
			$date = substr(date("F", mktime(0, 0, 0, $m, 10)),0,3).' '.$row['year_'];
			$data_month_Standard_No_CM[$date] = $row['CRQnum'];
		}
		sqlsrv_free_stmt( $stmt);
	...
	function drawAll() {         
		var dataTable = google.visualization.arrayToDataTable([           
			/*	['Week', 'Number of Changes'],
				...
				['12 2012',5,4,3,1,2,6],
			*/,'Standard with no CM'
			['Month', 'All Changes','Normal Authorized Changes','Incidents','Incidents due to Changes (IDC)','IDC % in Normal Authorized Changes'],  
		...
		var dataChart_All = google.visualization.arrayToDataTable([           
			...
			*/'Standard with No CM',
			...
			<?php	for($i=0;$i<count($data_month); $i++) {     
						$month = $data_month[$i]; //$data_month_Standard_No_CM[$month].",".
...
var options = { title : 'Trend of Incidents due to Changes',     
				...
				//series:{0:{color:"gray"},1:{color:"darkblue"},2:{targetAxisIndex:2,color:"green"},3:{targetAxisIndex:2,color:"yellow"},4:{targetAxisIndex:2,type:"bars",color:"orange"},5:{targetAxisIndex:1, type:"bars",color:"red"}}, 
