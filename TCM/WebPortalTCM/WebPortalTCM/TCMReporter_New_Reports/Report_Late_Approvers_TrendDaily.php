...
<?php
		...
		$sql0 = "SELECT datepart(Day,ch.[First Approval Date]) appDay,
				  ...
				  END hourInterval, CASE WHEN datepart(HOUR,ch.[First Approval Date]) < 14 THEN 1 ELSE 2 END orderList, count(*) CRQnum
							...
							GROUP BY  datepart(Day,ch.[First Approval Date]),
							  CASE WHEN datepart(HOUR,ch.[First Approval Date]) < 14 THEN 'Before 2:00 PM'
								 WHEN datepart(HOUR,ch.[First Approval Date]) = 14 THEN '2:00 - 3:00'
								 WHEN datepart(HOUR,ch.[First Approval Date]) = 15 THEN '3:00 - 4:00'
								 WHEN datepart(HOUR,ch.[First Approval Date]) = 16 THEN '4:00 - 5:00'
				...
				ORDER BY  datepart(Day,ch.[First Approval Date]),orderList ;	";
		
		$sql = "SELECT appDay, appDayOrder, [Before 2:00 PM] 'Before2:00',[2:00 - 3:00] '2:00-3:00',[3:00 - 4:00] '3:00-4:00',[4:00 - 5:00] '4:00-5:00',[After 5:00 PM] 'After5:00'
				  FROM (
					  SELECT DATENAME(dw,ch.[First Approval Date]) appDay, DATEPART(dw,ch.[First Approval Date]) appDayOrder,
					  CASE WHEN datepart(HOUR,ch.[First Approval Date]) < 14 THEN 'Before 2:00 PM'
									 WHEN datepart(HOUR,ch.[First Approval Date]) = 14 THEN '2:00 - 3:00'
									 WHEN datepart(HOUR,ch.[First Approval Date]) = 15 THEN '3:00 - 4:00'
									 WHEN datepart(HOUR,ch.[First Approval Date]) = 16 THEN '4:00 - 5:00'
					...
					GROUP BY DATENAME(dw,ch.[First Approval Date]) , DATEPART(dw,ch.[First Approval Date]),
							CASE WHEN datepart(HOUR,ch.[First Approval Date]) < 14 THEN 'Before 2:00 PM'
								 WHEN datepart(HOUR,ch.[First Approval Date]) = 14 THEN '2:00 - 3:00'
								 WHEN datepart(HOUR,ch.[First Approval Date]) = 15 THEN '3:00 - 4:00'
								 WHEN datepart(HOUR,ch.[First Approval Date]) = 16 THEN '4:00 - 5:00'
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
			...
			array_push($data_Day,$row['appDay']);
...
<script type="text/javascript">       
	...
	function drawAll() {         
		var dataTable = google.visualization.arrayToDataTable([           
			['Day', 'Before 2:00 PM', '2:00 - 3:00','3:00 - 4:00','4:00 - 5:00','After 5:00 PM'],           
			<?php 
				for($i=0;$i<count($data_Day); $i++) {             
						echo "['".$data_Day[$i]."',".$data_Desc[0][$i].",".$data_Desc[1][$i].",".$data_Desc[2][$i].",".$data_Desc[3][$i].",".$data_Desc[4][$i]."]"; 
		...
		var dataChart = google.visualization.arrayToDataTable([   
			['Day', '2:00 - 3:00','3:00 - 4:00','4:00 - 5:00','After 5:00 PM'],           
			<?php 
				for($i=0;$i<count($data_Day); $i++) {             
						echo "['".$data_Day[$i]."',".$data_Desc[1][$i].",".$data_Desc[2][$i].",".$data_Desc[3][$i].",".$data_Desc[4][$i]."]"; 
						if ($i<count($data_Day)-1) echo ",";	//add ',' to all except last element
					}  
			?>
			] ); 
		...
		//drawPieChart
		var options = { title: 'Daily Trend of Approvals for Normal Changes'  ,  chartArea:{left:40,top:40,bottom:120,width:"75%",height:"70%"}, 
