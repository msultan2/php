...
<?php // content="text/plain; charset=utf-8"
	...
	//$data_Val = array(); 
		...
		$sql = "SELECT cap.[Support_Company], YEAR(cap.Scheduled_Start_Date) year_,MONTH(cap.Scheduled_Start_Date) month_, count(*) CRQnum
			  FROM dbo.vw_Change_Approval_Details cap
			  WHERE Support_Company IN ('IT Operations', 'Network Engineering','Products & Services Delivery','Regional Operations','Service Management')
			  AND CRQ_Type = 'Normal' 
			  AND Status = 'Scheduled'
		...
		while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
			array_push($data_year_all,$row['year_']);
			array_push($data_dept_all,$row['Support_Company']);
			array_push($data_month_all,$row['month_']);
			array_push($data_Val_all,$row['CRQnum']);
		}
...
<script type="text/javascript">       
	...
	function drawAll() {         
		...
		dataTable.addColumn('number', 'Number of Changes');       
		dataTable.addRows([           
			<?php	for($i=0;$i<count($data_dept_all); $i++) {             
						echo "['".$data_dept_all[$i]."', '".date("F", mktime(0, 0, 0, $data_month_all[$i], 1))." " . $data_year_all[$i]."', ".$data_Val_all[$i]."]";
						if ($i<count($data_dept_all)-1) echo ",";	//add ',' to all except last element
					}
			?>
