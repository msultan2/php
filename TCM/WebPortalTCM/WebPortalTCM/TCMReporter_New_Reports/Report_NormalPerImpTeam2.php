...
<table><tr><td width="90%"><div class="body_text">Authorized Normal Changes per Implementing Team </div></td>
<td align=right >
<?php 
		$sql = "SELECT YEAR(cap.Scheduled_Start_Date) Scheduled_year,MONTH(cap.Scheduled_Start_Date) AS Scheduled_month,tsk.Assignee_Company,tsk.Assignee_Organization,Assignee_Group, count(*) CRQnum FROM  dbo.vw_Change_Approval_Details cap,dbo.tbl_Change_Task_Merged tsk WHERE cap.CRQ = tsk.Request_ID AND cap.Status NOT IN ('Draft', 'Request For Authorization') AND cap.CRQ_Type = 'Normal' GROUP BY YEAR(cap.Scheduled_Start_Date),MONTH(cap.Scheduled_Start_Date),tsk.Assignee_Company,tsk.Assignee_Organization,Assignee_Group ORDER BY YEAR(cap.Scheduled_Start_Date),MONTH(cap.Scheduled_Start_Date),tsk.Assignee_Organization;";
		$sql_encoded = str_replace(" ","|",str_replace("=","__EQUAL__",$sql));
		echo '<a href="excel.php?name=Monthly_Normal_Implemented&query='.$sql_encoded.'">';
	?>
...
<script type="text/javascript">       
	...
	function drawAll() {         
		...
		//drawPieChart
		var options = { title: 'Normal Changes'  ,  chartArea:{left:40,top:30,width:"70%",height:"70%"}, vAxis: {gridlines:{count:8}}, //maxValue:3, minValue:1},
						hAxis: {title: 'Number of Normal Scheduled Changes by top 5 Implementing Teams', titleTextStyle: {color: 'red'}, gridlines:{count:0}}   };          
