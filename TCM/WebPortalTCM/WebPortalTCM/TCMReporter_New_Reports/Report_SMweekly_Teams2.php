...
<?php
	...
	/* Specify the server and connection string attributes. */
		...
		$sql = "SELECT CASE WHEN tsk.Assignee_Company = 'Regional Operations' THEN 2
					   ...
					   CASE WHEN tsk.Assignee_Company = 'Service Management' THEN tsk.Assignee_Organization
				...
				AND dbo.DateOnly(ch.Scheduled_Start_Date) <= ( getdate() -1 ) AND dbo.DateOnly(ch.Scheduled_Start_Date) > getdate() - 8
				and ch.Status <> 'Request For Authorization'
				group by CASE WHEN tsk.Assignee_Company = 'Regional Operations' THEN 2
							WHEN tsk.Assignee_Company = 'Service Management' THEN 1
							ELSE 3 END,CASE WHEN tsk.Assignee_Company = 'Service Management' THEN tsk.Assignee_Organization
							WHEN tsk.Assignee_Company = 'Regional Operations' THEN 'Regional Operations'
							ELSE 'Others'
							END
...
<script type="text/javascript">       
	...
	function draw_ChangeFor(){
		var data = google.visualization.arrayToDataTable([           
			['Team', 'Normal Changes','Standard Changes'],           
		...
		var options_new = { 	title: 'NW CRQs Authorized per Team'  , 
					...
					seriesType: "bars",'isStacked':true,
					series:{0:{type:"bars"},1:{targetAxisIndex:2, type:"bars"}}, 
					vAxes:{2:{title:'Standard Changes',maxValue:1000,gridlines:{count:7}}},
