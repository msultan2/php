...
<?php 
		...
		//$sql = "SELECT cap.Support_Company,cap.Support_Organization,cap.Support_Group_Name,count(normal_cap.CRQ) Normal_CRQs,count(standard_cap.CRQ) Standard_CRQs,count(*) Total_CRQs FROM  dbo.tbl_Change_Approval_Details_Ver7_6 cap LEFT OUTER JOIN (SELECT * FROM  dbo.tbl_Change_Approval_Details_Ver7_6 WHERE CRQ_Type = 'Normal') normal_cap ON cap.CRQ = normal_cap.CRQ LEFT OUTER JOIN (SELECT * FROM  dbo.tbl_Change_Approval_Details_Ver7_6 WHERE CRQ_Type = 'Standard') standard_cap ON cap.CRQ = standard_cap.CRQ WHERE cap.Status != 'Draft' GROUP BY cap.Support_Company,cap.Support_Organization,cap.Support_Group_Name ORDER BY cap.Support_Company,cap.Support_Organization,cap.Support_Group_Name;";
